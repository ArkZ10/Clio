 ...
 * @lovable.dev/vite-tanstack-config injects these via Vite `define`, which only
 * substitutes the literal `import.meta.env.VITE_X` member expression -- a
 * dynamic `import.meta.env["VITE_X"]` is left unreplaced. So the declaration,
 * not the access style, is the right fix.
 */
